...
const RpgApiService: AxiosInstance = axios.create({
    baseURL: '/quests',
    headers: {
...
// Request interceptor
RpgApiService.interceptors.request.use(
    (config) => {
...
RpgApiService.interceptors.response.use(
...
export const get = <T>(url: string, config?: AxiosRequestConfig): Promise<ApiResponse<T>> => {
    ...
    return RpgApiService.get<ApiResponse<T>>(url, config).then(handleResponse).catch(handleError);
};

export const post = <T>(url: string, data?: any, config?: AxiosRequestConfig): Promise<ApiResponse<T>> =>
    RpgApiService.post<ApiResponse<T>>(url, data, config).then(handleResponse).catch(handleError);

export const put = <T>(url: string, data?: any, config?: AxiosRequestConfig): Promise<ApiResponse<T>> =>
    RpgApiService.put<ApiResponse<T>>(url, data, config).then(handleResponse).catch(handleError);

export const del = <T>(url: string, config?: AxiosRequestConfig): Promise<ApiResponse<T>> =>
    RpgApiService.delete<ApiResponse<T>>(url, config).then(handleResponse).catch(handleError);

export const patch = <T>(url: string, data?: any, config?: AxiosRequestConfig): Promise<ApiResponse<T>> =>
    RpgApiService.patch<ApiResponse<T>>(url, data, config).then(handleResponse).catch(handleError);
export default RpgApiService;