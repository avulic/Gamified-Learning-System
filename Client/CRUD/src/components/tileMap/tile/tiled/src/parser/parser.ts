...
export class TiledParser {
  ...
  constructor(private xml: string, private basePath: string = '') { }
  ...
  getImagePath(image: string) {
    return new URL(image, this.basePath).href
  }
  ...
  transform = (obj) => {
    ...
    let newObj = {
      ...
      ...TiledParser.propToNumber(attr, [
        'version', 'width', 'height', 'tilewidth', 'tileheight', 'nextlayerid',
        'nextobjectid', 'hexsidelength', 'opacity', 'x', 'y', 'offsetx', 'offsety',
        'startx', 'starty', 'id', 'firstgid', 'imageheight', 'imagewidth', 'margin',
        'columns', 'rows', 'tilecount', 'rotation', 'gid', 'tileid', 'duration',
        'parallaxx', 'parallaxy', 'repeatx', 'repeaty', 'pixelsize'
      ]),
      ...TiledParser.propToBool(attr, [
        'visible', 'infinite', 'locked', 'bold', 'italic', 'kerning', 'strikeout',
        'underline', 'wrap'
      ])
    }
  ...
  static decode(obj: { encoding: string, data: string }, size: number) {
    ...
    if (encoding == 'base64') {
      return atob(data.trim()).split('').map(char => char.charCodeAt(0))
    }
