...
<template>
    <div class="task-list p-4 bg-gray-50 rounded-lg">
        <div class="flex justify-between items-center mb-4">
            ...
            <SplitButton label="Add Task" :model="taskTypeMenuItems" @click="handleAddTask(TaskTypeEnum.QUESTION)"
                class="p-button-sm" />
        </div>

        <DataTable :value="tasks" v-model:selection="selectedTask" dataKey="id" :reorderableRows="true"
            @rowReorder="handleTaskReorder" class="p-datatable-sm" stripedRows>
            ...
            <Column field="title" header="Title">
                <template #body="{ data }">
                    <div class="flex flex-col gap-1">
                        ...
                        <Tag :value="getTaskTypeLabel(data.taskType)" :severity="getTaskTypeSeverity(data.taskType)" />
                    </div>
                </template>
            </Column>
            ...
            <Column style="width: 8rem">
                <template #body="{ data }">
                    <div class="flex gap-2">
                        <Button icon="pi pi-pencil" @click="editTask(data)" class="p-button-text p-button-sm" />
                        <Button icon="pi pi-trash" @click="confirmDeleteTask(data)"
        ...
        <!-- Task Editor Dialog -->
        <Dialog v-model:visible="showTaskDialog" :header="taskDialogTitle" :style="{ width: '70vw' }" modal>
            <TaskEditor v-if="showTaskDialog" :task="selectedTask" :task-type="selectedTask?.taskType || newTaskType"
                @save="handleTaskSave" @cancel="closeTaskDialog" />
        </Dialog>
...
import TaskEditor from '@/features/tasks/components/task/TaskEditor.vue';
