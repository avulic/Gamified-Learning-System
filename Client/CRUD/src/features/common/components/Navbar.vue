<template>
    <nav class="bg-app-bg px-4 py-3">
        <div class="max-w-7xl mx-auto flex justify-between items-center">
            <div class="flex items-center">
                <RouterLink :to="{ name: 'home' }" aria-label="Company" title="Company"
                    class="inline-flex items-center mr-8">
                    <svg class="w-8 text-teal-accent-400" viewBox="0 0 24 24" stroke-linejoin="round" stroke-width="2"
                        stroke-linecap="round" stroke-miterlimit="10" stroke="currentColor" fill="none">
                        <rect x="3" y="1" width="7" height="12"></rect>
                        <rect x="3" y="17" width="7" height="6"></rect>
                        <rect x="14" y="1" width="7" height="6"></rect>
                        <rect x="14" y="11" width="7" height="12"></rect>
                    </svg>
                    <span class="ml-2 text-xl font-bold tracking-wide text-app-text uppercase">Company</span>

                </RouterLink>
                <ul class="flex items-center hidden space-x-8 lg:flex">
                    <li>
                        <RouterLink :to="{ name: 'home' }" aria-label="Users" title="Users"
                            class="font-medium tracking-wide text-app-text transition-colors duration-200 hover:text-teal-accent-400">
                            Home</RouterLink>
                    </li>
                    <li>
                        <RouterLink :to="{ name: 'dashboard' }"
                            class="font-medium tracking-wide text-app-text transition-colors duration-200 hover:text-teal-accent-400">
                            Dashboard
                        </RouterLink>
                        <RouterLink :to="{ name: 'questMapper' }"
                            class="font-medium tracking-wide text-app-text transition-colors duration-200 hover:text-teal-accent-400">
                            QuestMapper
                        </RouterLink>
                    </li>
                    <li v-if="isStudent">
                        <RouterLink :to="{ name: 'courses' }"
                            class="font-medium tracking-wide text-app-text transition-colors duration-200 hover:text-teal-accent-400">
                            My Courses
                        </RouterLink>
                    </li>
                    <li v-if="currentUser">
                        <RouterLink :to="{ name: 'profile' }"
                            class="font-medium tracking-wide text-app-text transition-colors duration-200 hover:text-teal-accent-400">
                            Profile
                        </RouterLink>
                    </li>
                </ul>
            </div>
            <ul v-if="!currentUser" class="flex items-center hidden space-x-8 lg:flex">
                <li>
                    <RouterLink :to="{ name: 'signin' }" aria-label="signin" title="signin"
                        class="font-medium tracking-wide text-app-text transition-colors duration-200 hover:text-teal-accent-400">
                        Sign In</RouterLink>
                </li>
                <li>
                    <RouterLink :to="{ name: 'signup' }" aria-label="signup" title="signup"
                        class="font-medium tracking-wide text-app-text transition-colors duration-200 hover:text-teal-accent-400">
                        Sign Up</RouterLink>
                </li>
            </ul>
            <ul v-else class="flex items-center hidden space-x-8 lg:flex">
                <li>
                    <a @click="logout"
                        class="font-medium tracking-wide text-app-text transition-colors duration-200 hover:text-teal-accent-400">Logout</a>
                </li>
                <li>
                    <Button :icon="isDark ? 'pi pi-sun' : 'pi pi-moon'" rounded text @click="toggle" />
                </li>
            </ul>

            <div class="lg:hidden">
                <button aria-label="Open Menu" title="Open Menu"
                    class="p-2 -mr-1 transition duration-200 rounded focus:outline-none focus:shadow-outline"
                    @click="isMenuOpen = true">
                    <svg class="w-5 text-gray-600" viewBox="0 0 24 24">
                        <path fill="currentColor"
                            d="M23,13H1c-0.6,0-1-0.4-1-1s0.4-1,1-1h22c0.6,0,1,0.4,1,1S23.6,13,23,13z">
                        </path>
                        <path fill="currentColor" d="M23,6H1C0.4,6,0,5.6,0,5s0.4-1,1-1h22c0.6,0,1,0.4,1,1S23.6,6,23,6z">
                        </path>
                        <path fill="currentColor"
                            d="M23,20H1c-0.6,0-1-0.4-1-1s0.4-1,1-1h22c0.6,0,1,0.4,1,1S23.6,20,23,20z">
                        </path>
                    </svg>
                </button>
                <div v-if="isMenuOpen" class="absolute top-0 left-0 w-full">
                    <div class="p-5 bg-white border rounded shadow-sm">
                        <div class="flex items-center justify-between mb-4">
                            <div>
                                <a href="/" aria-label="Company" title="Company" class="inline-flex items-center">
                                    <svg class="w-8 text-deep-purple-accent-400" viewBox="0 0 24 24"
                                        stroke-linejoin="round" stroke-width="2" stroke-linecap="round"
                                        stroke-miterlimit="10" stroke="currentColor" fill="none">
                                        <rect x="3" y="1" width="7" height="12"></rect>
                                        <rect x="3" y="17" width="7" height="6"></rect>
                                        <rect x="14" y="1" width="7" height="6"></rect>
                                        <rect x="14" y="11" width="7" height="12"></rect>
                                    </svg>
                                    <span
                                        class="ml-2 text-xl font-bold tracking-wide text-gray-800 uppercase">Company</span>
                                </a>
                            </div>
                            <div>
                                <button aria-label="Close Menu" title="Close Menu"
                                    class="p-2 -mt-2 -mr-2 transition duration-200 rounded hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                                    @click="isMenuOpen = false">
                                    <svg class="w-5 text-gray-600" viewBox="0 0 24 24">
                                        <path fill="currentColor"
                                            d="M19.7,4.3c-0.4-0.4-1-0.4-1.4,0L12,10.6L5.7,4.3c-0.4-0.4-1-0.4-1.4,0s-0.4,1,0,1.4l6.3,6.3l-6.3,6.3 c-0.4,0.4-0.4,1,0,1.4C4.5,19.9,4.7,20,5,20s0.5-0.1,0.7-0.3l6.3-6.3l6.3,6.3c0.2,0.2,0.5,0.3,0.7,0.3s0.5-0.1,0.7-0.3 c0.4-0.4,0.4-1,0-1.4L13.4,12l6.3-6.3C20.1,5.3,20.1,4.7,19.7,4.3z">
                                        </path>
                                    </svg>
                                </button>
                            </div>
                        </div>
                        <nav>
                            <ul class="space-y-4">
                                <li>
                                    <a href="/" aria-label="Our product" title="Our product"
                                        class="font-medium tracking-wide text-gray-700 transition-colors duration-200 hover:text-deep-purple-accent-400">Product</a>
                                </li>
                                <li>
                                    <a href="/" aria-label="Our product" title="Our product"
                                        class="font-medium tracking-wide text-gray-700 transition-colors duration-200 hover:text-deep-purple-accent-400">Features</a>
                                </li>
                                <li>
                                    <a href="/" aria-label="Product pricing" title="Product pricing"
                                        class="font-medium tracking-wide text-gray-700 transition-colors duration-200 hover:text-deep-purple-accent-400">Pricing</a>
                                </li>
                                <li>
                                    <a href="/" aria-label="About us" title="About us"
                                        class="font-medium tracking-wide text-gray-700 transition-colors duration-200 hover:text-deep-purple-accent-400">About
                                        us</a>
                                </li>
                                <li>
                                    <a href="/" aria-label="Sign in" title="Sign in"
                                        class="font-medium tracking-wide text-gray-700 transition-colors duration-200 hover:text-deep-purple-accent-400">Sign
                                        in</a>
                                </li>
                                <li>
                                    <a href="/"
                                        class="inline-flex items-center justify-center w-full h-12 px-6 font-medium tracking-wide text-white transition duration-200 rounded shadow-md bg-deep-purple-accent-400 hover:bg-deep-purple-accent-700 focus:shadow-outline focus:outline-none"
                                        aria-label="Sign up" title="Sign up">
                                        Sign up
                                    </a>
                                </li>
                            </ul>
                        </nav>
                    </div>
                </div>
            </div>
        </div>
    </nav>
</template>

<script setup lang="ts">
import { watch, computed, onBeforeMount, ref, onMounted, onUnmounted } from 'vue'
import { useRouter } from 'vue-router'
import AuthService from '@/services/AuthService';
import type User from '@/types/User/User';
import { Observable } from 'rxjs';
import { RoleEnum } from '@/types/enums';

const router = useRouter();

var isMenuOpen = false

var currentUser = ref<User | null>(null);
var subscription: any = null;

onMounted(() => {
    AuthService.loadUserFromLocalStorage();

    // Use the public observable method
    subscription = AuthService.getCurrentUserObservable().subscribe((user: User | null) => {
        currentUser.value = user;
    });
});

onUnmounted(() => {
    if (subscription) subscription.unsubscribe();
});


const isAdmin = computed(() => {
    return currentUser.value?.roles?.some(role => role.name === RoleEnum.ADMIN) ?? false
})

const isProfessor = computed(() => {
    return currentUser.value?.roles?.some(role => role.name === RoleEnum.PROFESSOR) ?? false
})

const isStudent = computed(() => {
    return currentUser.value?.roles?.some(role => role.name === RoleEnum.STUDENT) ?? false
})

const logout = () => {
    AuthService.userLogOut();
    router.push({ name: 'signin' });
}

const isDark = computed(() => document.documentElement.classList.contains('dark'))
const toggle = () => {
    document.documentElement.classList.toggle('dark')
    localStorage.theme = isDark.value ? 'light' : 'dark'
}
</script>