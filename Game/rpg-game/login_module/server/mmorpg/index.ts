...
@RpgModule<RpgServer>({
    ...
    engine: {
        onStart(engine: RpgServerEngine) {
            ...
            const { mongodb } =  engine.globalConfig.login_module
    ...
    player: {
        ...
        onConnected(player: RpgPlayer) {
            ...
            const subjectsGui = player.gui('rpgSelectSubject')
            ...
            gui.on('start-game', async (data) => {
                ...
                if (user.data === undefined) {
                    ...
                    if (start) {
                        if (start.hitbox) 
                            player.setHitbox(...(start.hitbox as [number, number]))
                        if (start.graphic) 
